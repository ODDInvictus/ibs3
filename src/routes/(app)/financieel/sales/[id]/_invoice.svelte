<script lang="ts">
	...
	const IBAN = 'LT98 3250 0197 5500 8508';
...
<div id="invoice">
	<main>
		...
		<div class="top">
			<div>
				<p><b>{invoice.to.name}</b></p>
				{#if invoice.tav}
					<p>T.a.v. {invoice.tav}</p>
				{/if}
				{#if invoice.to.FinancialPersonDataOther}
					<p>{invoice.to.FinancialPersonDataOther.address}</p>
					<p>
						{invoice.to.FinancialPersonDataOther.postalCode}
						{invoice.to.FinancialPersonDataOther.city}
					</p>
				{:else if invoice.to.FinancialPersonDataUser}
					<p>{invoice.to.FinancialPersonDataUser.user.personalEmail}</p>
				{/if}
			</div>
			<div>
