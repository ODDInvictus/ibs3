...
export const POST = (async ({ request, locals }) => {
  ...
  if (body.id && body.userId) {
    ...
    try {
      ...
      return new Response(JSON.stringify({ message: 'Dit lid zit al in deze commissie' }), { status: 409 })
    } catch (err) {
      console.error('[Admin/Commissie]', err)
  ...
  return new Response(JSON.stringify({ message: 'Commissie of gebruiker niet gevonden' }), { status: 404 })
}) satisfies RequestHandler;
