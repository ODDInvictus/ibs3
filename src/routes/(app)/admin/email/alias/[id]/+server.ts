  ...
  try {
    if (body.id && body.type) {
      if (body.type === 'custom' && body.address) {
        const alias = await db.emailAlias.findFirst({ where: { id: body.id }, include: { EmailContact: true } })

        if (alias) {
          await db.emailContact.update({ where: { id: alias.EmailContact[0].id }, data: { address: body.address } })

          return new Response(JSON.stringify({ message: 'Alias geupate!' }), { status: 200 })
        }
      } else if (body.type === 'user' && body.uid) {
        const alias = await db.emailAlias.findFirst({ where: { id: body.id }, include: { EmailAliasUser: true } })
    ...
    console.log(err)
    return new Response(JSON.stringify({ message: 'Alias niet geupate wegens een server fout!' }), { status: 500 })
  }
