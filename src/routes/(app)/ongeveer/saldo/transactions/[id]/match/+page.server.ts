	...
	const data = {
		rows: transaction.Transaction.TransactionMatchRow.map((r) => {
			if (!r.journalId) {
				throw error(
					500,
					'JournalId is missing, while a saldo transaction should always be matched to a journal'
				);
			}
			return {
...
export const actions = {
	default: async (event) => {
		...
		const total = form.data.rows
			.reduce((acc, row) => acc.add(new Decimal(row.amount)), new Decimal(0))
			.add(fromBank);
		...
		for (let i = 0; i < form.data.rows.length; i++) {
			...
			// Max amount to match
			const journalTotal = journal.Rows.reduce(
