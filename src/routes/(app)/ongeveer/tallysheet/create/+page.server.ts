...
function groupByPersonAndGetProducts(data: { financialPersonId: number; productId: number; amount: number }[]) {
	const result = new Map<number, { productId: number; amount: number }[]>()
	const products = new Set<number>()
	for (const { financialPersonId, productId, amount } of data) {
		if (Number.isNaN(amount ?? undefined) || Number.isNaN(productId ?? undefined) || Number.isNaN(financialPersonId ?? undefined)) continue
...
export const actions = {
	default: async event => {
		...
		try {
			...
			const [tallySheet, products, ledgers] = await Promise.all([tallySheetPromise, productsPromise, ledgersPromise])
