...
export const DELETE: RequestHandler = async ({ params, locals, url }) => {
	...
	if (!authorization(locals.roles)) return new Response('Je moet senaat of financie zijn', { status: 403 })
	...
	if (status === 'PAID')
		return new Response('Aankoop is gematched aan een banktransactie, unmatch de banktransactie voor je de aankoop kan verwijderen', {
			status: 409,
		})
...
export const PATCH: RequestHandler = async ({ params, locals }) => {
	...
	const total = journal.Rows.reduce((acc, { amount, price }) => acc.add(new Decimal(amount).mul(price)), new Decimal(0))
	const matched = journal.TransactionMatchRow.reduce((acc, { amount }) => acc.add(amount), new Decimal(0))
