...
export async function processPhotos() {
	...
	for (const photo of photos) {
		...
		try {
			...
			await sharp(buf).resize(rw, rh).avif({ quality }).toFile(outPath('large'))
			...
			await sharp(buf).resize(rw, rh).avif({ quality }).toFile(outPath('medium'))
			...
			await sharp(buf).resize(rw, rh).avif({ quality }).toFile(outPath('small'))
