...
export async function syncLDAPUsers() {
	...
	for (const group of authentikGroups) {
		...
		for (const member of group.users_obj) {
			...
			const ibsGroupMember = ibsGroupMembers.find(
				ibsGroupMember => ibsGroupMember.userId === ibsUser.id && ibsGroupMember.committeeId === ibsGroup?.id,
			)
	...
	for (const group of ibsGroups) {
		const financialPersonData = financialPeopleDataCommittee.find(
			financialPersonDataCommittee => financialPersonDataCommittee.committeeId === group.id,
		)
